#!/usr/bin/env python3
"""Fetch Michigan turnout by county (turnout-only; MI has no party registration).

Source: MI Secretary of State Voter Participation Dashboard (Power BI), which
publishes absentee/early voting counts by county, updated daily starting 45 days
before Election Day. MI does not distinguish mail vs in-person early. Public
access is the Power BI dashboard; the raw weekday county file has no public URL.

STATUS: staged skeleton. Writes an empty (0) turnout-only snapshot until a
verified live 2026 feed is wired; the Friday rollout routine captures the Power
BI querydata endpoint (or the county file) and finalizes the parser once MI's
dashboard populates for the general (~Sept 19 2026). Kept in the same schema as
Virginia's turnout-only connector so the front end treats it identically (green
turnout-intensity map, no party UI).

Run:  python scripts/mi_update.py [--force]
"""
import os
import sys
import json
...
def fetch_counties(cfg):
    """Return {county_name: {fips, cast_total, registered}} from the live MI feed.

    PENDING: no verified public machine-readable 2026 feed off-season (Power BI
    dashboard only). Returns {} (empty snapshot) for now; the rollout routine
    wires the live parser in Oct.
    """
    return {}
...
def main():
    ...
    snap = {
        ...
        "partisan": False,
        "source": {"primary": "MI Secretary of State Voter Participation Dashboard (turnout-only; MI has no party registration)"},
